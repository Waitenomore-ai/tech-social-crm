...
(() => {
  ...
  const load=()=>{
    ...
    if(!document.querySelector('link[data-v5-calendar-layout-fix]')){const link=document.createElement('link');link.rel='stylesheet';link.href=`calendar-layout-fix.css?v=${Date.now()}`;link.dataset.v5CalendarLayoutFix='1';document.head.appendChild(link);}
